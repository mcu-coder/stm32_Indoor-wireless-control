...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuekongtiao != cur->valuekongtiao)
    {
        GIZWITS_LOG("valuekongtiao Changed\n");
        ret = 1;
    }
    ...
    if(last->valueliangdu != cur->valueliangdu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueliangdu Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(kongtiao_BYTEOFFSET,kongtiao_BITOFFSET,kongtiao_LEN,(uint8_t *)devStatusPtr,dataPoints->valuekongtiao);
    ...
    devStatusPtr->valueliangdu = gizY2X(liangdu_RATIO,  liangdu_ADDITION, dataPoints->valueliangdu); 
